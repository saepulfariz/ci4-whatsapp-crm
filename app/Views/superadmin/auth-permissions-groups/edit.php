...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">Edit Auth Permission Group</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
...
<section class="content">
    <div class="container-fluid">
        ...
        <form action="<?= base_url($link . '/' . $permission_group->id); ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field(); ?>
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <select type="text" class="form-control <?= ($error = validation_show_error('group_id')) ? 'border-danger' : ((old('group_id')) ? 'border-success' : ''); ?> " value="<?= old('group_id'); ?>" id="group_id" name="group_id">
                                    <?php foreach ($groups as $group): ?>
                                        <?php if (old('group_id')): ?>
                                            <?php if (old('group_id') == $group->id): ?>
                                                <option selected value="<?= $group->id; ?>"><?= $group->title; ?></option>
                                            <?php else: ?>
                                                <option value="<?= $group->id; ?>"><?= $group->title; ?></option>
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <?php if ($permission_group->group_id == $group->id): ?>
                                                <option selected value="<?= $group->id; ?>"><?= $group->title; ?></option>
                                            <?php else: ?>
                                                <option value="<?= $group->id; ?>"><?= $group->title; ?></option>
                                            <?php endif; ?>
                            ...
                            <div class="form-group">
                                ...
                                <input list="permissions" type="text" class="form-control <?= ($error = validation_show_error('permission')) ? 'border-danger' : ((old('permission')) ? 'border-success' : ''); ?> " value="<?= old('permission', $permission_group->permission); ?>" id="permission" name="permission">
                            </div>
                            <?= ($error) ? '<div class="error text-danger mb-2" style="margin-top: -15px">' . $error . '</div>' : ''; ?>
