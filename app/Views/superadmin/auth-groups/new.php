...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">New Auth Group</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control <?= ($error = validation_show_error('description')) ? 'border-danger' : ((old('description')) ? 'border-success' : ''); ?>" name="description" id="description"><?= old('description'); ?></textarea>
                            </div>
                            <?= ($error) ? '<div class="error text-danger mb-2" style="margin-top: -15px">' . $error . '</div>' : ''; ?>
                    ...
                    <div class=" table-wrapper table-responsive">
                        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-3">
                            <?php foreach ($permissions as $group => $items): ?>
                                <div class="col">
                                    <div class="card card-body rounded-3 p-4 mb-4">
                                        <h2 class="h5 text-dark mb-4 border-bottom pb-2"><?= ucfirst($group) ?></h2>
                                        <div class="space-y-2">
                                            <?php foreach ($items as $permission): ?>
                                                <?php
                                                $oldPermissions = old('permissions') ?? [];
                                                $checked = in_array($permission->name, $oldPermissions) ? 'checked' : '';
                                                ?>
