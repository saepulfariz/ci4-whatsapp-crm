...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0"><?= temp_lang('app.edit'); ?> <?= temp_lang('products.product'); ?></h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
...
<section class="content">
    <div class="container-fluid">
        ...
        <form action="<?= base_url($link . '/' . esc($product->id)); ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field(); ?>
                    ...
                    <div class="card">
                        <div class="card-body">
                                ...
                                <select type="text" class="form-control <?= ($error = validation_show_error('category_id')) ? 'border-danger' : ((old('category_id')) ? 'border-success' : ''); ?> " value="<?= old('category_id'); ?>" id="category_id" name="category_id">
                                    <?php foreach ($categories as $category): ?>
                                        <?php if (old('category_id')): ?>
                                            ...
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <?php if (esc($product->category_id) == $category->id): ?>
                                                <option selected value="<?= $category->id; ?>"><?= $category->name; ?></option>
                                            <?php else: ?>
                                                <option value="<?= $category->id; ?>"><?= $category->name; ?></option>
                                            <?php endif; ?>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control <?= ($error = validation_show_error('name')) ? 'border-danger' : ((old('name')) ? 'border-success' : ''); ?>" id="name" name="name" placeholder="name" value="<?= old('name', esc($product->name)); ?>">
                            </div>
                            <?= ($error) ? '<div class="error text-danger mb-2" style="margin-top: -15px">' . $error . '</div>' : ''; ?>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" step="0.1" class="form-control <?= ($error = validation_show_error('price')) ? 'border-danger' : ((old('price')) ? 'border-success' : ''); ?>" id="price" name="price" placeholder="price" value="<?= old('price', esc($product->price)); ?>">
                            </div>
                            <?= ($error) ? '<div class="error text-danger mb-2" style="margin-top: -15px">' . $error . '</div>' : ''; ?>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control <?= ($error = validation_show_error('qty')) ? 'border-danger' : ((old('qty')) ? 'border-success' : ''); ?>" id="qty" name="qty" placeholder="qty" value="<?= old('qty', esc($product->qty)); ?>">
                            </div>
                            <?= ($error) ? '<div class="error text-danger mb-2" style="margin-top: -15px">' . $error . '</div>' : ''; ?>
                            ...
                            <div class="form-group">
                                ...
                                <div id="imagePreview">
                                    <img class="img-thumbnail d-block mb-2" width="100" src="<?= asset_url(); ?>uploads/products/<?= esc($product->image); ?>" alt="">
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control <?= ($error = validation_show_error('description')) ? 'border-danger' : ((old('description')) ? 'border-success' : ''); ?>" id="description" name="description" placeholder="description"><?= old('description', $product->description); ?></textarea>
                            </div>
                            <?= ($error) ? '<div class="error text-danger mb-2" style="margin-top: -15px">' . $error . '</div>' : ''; ?>
